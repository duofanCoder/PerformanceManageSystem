<template>
  <n-card title="分组管理" class="h-full shadow-sm rounded-16px">
    <n-space vertical>
  ...
  <n-modal v-model:show="showModalRef">
    <n-card style="width: 600px" title="分组管理" :bordered="false" size="huge" role="dialog" aria-modal="true">
      <n-form
        ref="formRef"
        :model="model"
      ...
      >
        ...
        <n-form-item label="成员" path="userList">
          <n-space>
            <n-table :bordered="false" :single-line="false">
              <thead>
                <tr>
                  <th class="w-30">姓名</th>
                  <th class="w-30">用户名</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="item in model.userList" :key="item.id">
...
import { fetchQueryGroup, fetchRemoveGroup, fetchSaveGroup, fetchUpdateGroup } from '@/service';
import { isUndefined } from '@/utils';

const queryKeyRef = ref('');
const model = ref<Partial<Dto.Group>>({});
const columns = [
  {
    type: 'selection',
  },
...
const data = ref<Array<Dto.Group>>([]);
const condition = reactive<Partial<Condition.Common>>({});
const reload = () => {
  fetchQueryGroup(condition).then((res) => {
...
const clickEvent = (type: string) => {
  ...
  switch (type) {
    ...
    case 'remove': {
      fetchRemoveGroup(checkedRowKeysRef.value).then((res) => {
...
const submitClickEvent = () => {
  switch (submitClickEventRef.value) {
    case 'update': {
      fetchUpdateGroup(model.value).then((res) => {
        reload();
    ...
    case 'add': {
      fetchSaveGroup(model.value).then((res) => {
        reload();
        showModalRef.value = false;
