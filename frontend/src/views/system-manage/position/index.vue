<template>
	<n-card title="职务管理" class="h-full shadow-sm rounded-16px">
		<n-space vertical>
	...
	<n-modal v-model:show="showModalRef">
		<n-card style="width: 600px" title="职务管理" :bordered="false" size="huge" role="dialog" aria-modal="true">
			<n-form
				ref="formRef"
				:model="model"
...
import { fetchQueryPosition, fetchRemovePosition, fetchSavePosition, fetchUpdatePosition } from "@/service";

const queryKeyRef = ref('');
const model = ref<Partial<Dto.Position>>({});
const columns = [
	{
		type: 'selection',
	},
...
const data = ref<Array<Dto.Position>>([]);
const condition = reactive<Partial<Condition.Common>>({});
const reload = () => {
	fetchQueryPosition(condition).then((res) => {
...
const clickEvent = (type: string) => {
	...
	switch (type) {
		...
		case 'remove': {
			fetchRemovePosition(checkedRowKeysRef.value).then((res) => {
...
const submitClickEvent = () => {
	switch (submitClickEventRef.value) {
		case 'update': {
			fetchUpdatePosition(model.value).then((res) => {
				reload();
		...
		case 'add': {
			fetchSavePosition(model.value).then((res) => {
				reload();
				showModalRef.value=false;
