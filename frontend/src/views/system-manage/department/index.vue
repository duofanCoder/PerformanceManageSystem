<template>
  <n-card title="部门管理" class="h-full shadow-sm rounded-16px">
    <n-space vertical>
  ...
  <n-modal v-model:show="showModalRef">
    <n-card style="width: 600px" title="部门管理" :bordered="false" size="huge" role="dialog" aria-modal="true">
      <n-form
        ref="formRef"
        :model="model"
...
import { fetchQueryDepartment, fetchRemoveDepartment, fetchSaveDepartment, fetchUpdateDepartment } from '@/service';

const queryKeyRef = ref('');
const model = ref<Partial<Dto.Department>>({});
const columns = [
  {
    type: 'selection',
  },
...
const data = ref<Array<Dto.Department>>([]);
const condition = reactive<Partial<Condition.Common>>({});
const reload = () => {
  fetchQueryDepartment(condition).then((res) => {
...
const clickEvent = (type: string) => {
  ...
  switch (type) {
    ...
    case 'remove': {
      fetchRemoveDepartment(checkedRowKeysRef.value).then((res) => {
...
const submitClickEvent = () => {
  switch (submitClickEventRef.value) {
    case 'update': {
      fetchUpdateDepartment(model.value).then((res) => {
        reload();
    ...
    case 'add': {
      fetchSaveDepartment(model.value).then((res) => {
        reload();
        showModalRef.value = false;
