<template>
  <n-card title="评价方式管理" class="h-full shadow-sm rounded-16px">
    <n-space vertical>
  ...
  <n-modal v-model:show="showModalRef">
    <n-card style="width: 600px" title="评价方式管理" :bordered="false" size="huge" role="dialog" aria-modal="true">
      <n-form
        ref="formRef"
        :model="model"
...
import { fetchQueryMethod, fetchRemoveMethod, fetchSaveMethod, fetchUpdateMethod } from "@/service";

const queryKeyRef = ref('');
const model = ref<Partial<Dto.Method>>({});
const columns = [
  {
    type: 'selection',
  },
...
const data = ref<Array<Dto.Method>>([]);
const condition = reactive<Partial<Condition.Common>>({});
const reload = () => {
  fetchQueryMethod(condition).then((res) => {
...
const clickEvent = (type: string) => {
  ...
  switch (type) {
    ...
    case 'remove': {
      fetchRemoveMethod(checkedRowKeysRef.value).then((res) => {
...
const submitClickEvent = () => {
  switch (submitClickEventRef.value) {
    case 'update': {
      fetchUpdateMethod(model.value).then((res) => {
        reload();
    ...
    case 'add': {
			fetchSaveMethod(model.value).then((res) => {
				reload();
				showModalRef.value=false;
