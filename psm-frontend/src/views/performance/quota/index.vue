<template>
  <n-card title="指标管理" class="h-full shadow-sm rounded-16px">
    <n-space vertical>
  ...
  <n-modal v-model:show="showModalRef">
    <n-card style="width: 600px" title="指标管理" :bordered="false" size="huge" role="dialog" aria-modal="true">
      <n-form
        ref="formRef"
        :model="model"
      ...
      >
        ...
        <n-form-item label="分值" path="score">
          <n-input-number v-model:value="model.score" placeholder="请输入分值" />
        </n-form-item>
        <n-form-item label="权重" path="description">
          <n-input-number v-model:value="model.weight" placeholder="请输入权重" />
        </n-form-item>
...
import { fetchQueryQuota, fetchRemoveQuota, fetchSaveQuota, fetchUpdateQuota } from '@/service';

const queryKeyRef = ref('');
const model = ref<Partial<Dto.Quota>>({});
const columns = [
  {
    type: 'selection',
  },
...
const data = ref<Array<Dto.Quota>>([]);
const condition = reactive<Partial<Condition.Common>>({});
const reload = () => {
  fetchQueryQuota(condition).then((res) => {
...
const clickEvent = (type: string) => {
  ...
  switch (type) {
    ...
    case 'remove': {
      fetchRemoveQuota(checkedRowKeysRef.value).then((res) => {
...
const submitClickEvent = () => {
  switch (submitClickEventRef.value) {
    case 'update': {
      fetchUpdateQuota(model.value).then((res) => {
        reload();
    ...
    case 'add': {
      fetchSaveQuota(model.value).then((res) => {
        reload();
        showModalRef.value = false;
